...
class MongoDb {
    ...
    public function __construct($params = array()) {
        ...
        if (!extension_loaded('mongo')) {
            Core::halt('The mongo extension to be loaded!');
        }

        //参数分析
        if (!$params || !is_array($params)) {
            //加载数据库配置文件.
            $params = config::get('MONGODB');
        }

        $params = is_array($params) ? $params + $this->_defaultConfig : $this->_defaultConfig;
        if (!isset($params['dbname']) || !$params['dbname']) {
            Core::halt('The file of MongoDB config is error, dbname is not found!');
        }
        ...
        try {
            ...
            //抛出异常信息
            throw new newexception('MongoDb connect error!<br/>' . $exception->getMessage(), $exception->getCode());
        }
    }
